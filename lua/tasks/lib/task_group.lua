...
function TaskGroup:run_group_level(tasks)
    ...
    for _, task in ipairs(tasks) do
        ...
        task.cancel_token:prepend(self.cancel_token)

        task:on_finish(function()
            print("task finished with status:", task:get_id(), task:get_state())
...
function TaskGroup:run()
    ...
    for _, tasks in ipairs(self.group) do
        if not self.cancel_token:is_cancelled() then
            current_level = tasks
            self:run_group_level(tasks)
        else
            for _, task in ipairs(tasks) do
                print("request task id to stop:", task:get_id())
                task:request_stop()
            end
        end
    end
